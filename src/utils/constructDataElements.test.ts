...
import { constructDataElementsForShape, fillGapInDataElements, fillMultiDimensionalArray } from "./constructDataElements";
...
describe('fillMultiDimensionalArray', () => {
  ...
  it('creates a flat array for single dimension', () => {
    expect(fillMultiDimensionalArray([3], [1], 0)).toEqual([new DataElement(0), new DataElement(1), new DataElement(2)]);
  });
...
describe('fillGapInDataElements', () => {
  ...
  it('should handle an array with no gaps', () => {
    const elements = [new DataElement(1), new DataElement(2), new DataElement(3)];
    const result = fillGapInDataElements(elements);
  ...
  it('should handle multiple gaps correctly', () => {
    const elements = [new DataElement(1), new DataElement(5), new DataElement(10)];
    const result = fillGapInDataElements(elements);
    expect(result).toEqual([
      new DataElement(1),
      new MemorySlot(2),
